...
function LeaderboardPage() {
  ...
  const rankedWards =
    leaderboard?.filter((w) => w.rankPosition != null && w.rankPosition > 0) ?? [];
  const top3 = rankedWards.slice(0, 3);
  const rest = rankedWards.slice(3);
  const unrankedWards =
    leaderboard?.filter(
      (w) => w.rankPosition == null || w.rankPosition === 0 || w.totalFeedbacks < 5,
    ) ?? [];
      ...
      <section className="relative h-[240px] w-full overflow-hidden bg-[#1e3a8a]">
        ...
        <div className="relative h-full max-w-[1440px] mx-auto px-6 md:px-10 flex flex-col justify-center pb-6">
          ...
          <p className="max-w-2xl text-sm md:text-base text-blue-100 font-medium leading-relaxed">
            Đánh giá hiệu quả tiếp nhận, xử lý phản ánh của các phường dựa trên tốc độ, tỷ lệ giải
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 mt-8 mb-8 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          >
            <Trophy
              size={18}
              className={activeTab === "top" ? "text-[#2563EB]" : "text-slate-400"}
            />
            Top Phường
            {activeTab === "top" && (
              <div className="absolute bottom-0 left-0 w-full h-[3px] bg-[#2563EB] rounded-t-full" />
            )}
          </button>
          <button
          ...
          >
            <AlertCircle
              size={18}
              className={activeTab === "unranked" ? "text-[#2563EB]" : "text-slate-400"}
            />
            Chưa đủ dữ liệu
            {activeTab === "unranked" && (
              <div className="absolute bottom-0 left-0 w-full h-[3px] bg-[#2563EB] rounded-t-full" />
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-10">
        {activeTab === "map" && (
          <div className="bg-white p-6 rounded-[18px] border border-slate-200 shadow-sm mt-8">
            <h2 className="font-sans text-lg font-black text-[#1E3A8A] mb-4">
              Bản đồ nhiệt năng lực các phường (Đà Nẵng)
            </h2>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <p className="text-sm text-slate-500 font-medium">
                Mức độ hoàn thành điểm đánh giá năng lực:
              </p>
        ...
        {activeTab === "unranked" && (
          <div className="bg-white p-6 rounded-[18px] border border-slate-200 shadow-sm mt-8">
            <h2 className="font-sans text-lg font-black text-[#1E3A8A] mb-4">
              Phường chưa đủ dữ liệu
            </h2>
            <p className="text-sm text-slate-500 mb-6 font-medium">
              Các phường có ít hơn 5 phản ánh trong tháng sẽ không được xếp hạng để đảm bảo tính
              khách quan.
            </p>
            {unrankedWards.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {unrankedWards.map((w) => (
                  <div
                    key={w.wardId}
                    className="border border-slate-200 p-4 rounded-[16px] shadow-sm hover:shadow-md cursor-pointer transition-shadow"
                ...
                {top3[1] && (
                  <div className="bg-white rounded-[18px] border border-slate-200 shadow-sm p-6 hover:shadow-md transition-shadow relative overflow-hidden order-2 lg:order-1 h-[280px] flex flex-col">
                    ...
                    <div className="flex justify-between items-start mb-6 mt-2 ml-8">
                      <div>
                        <h3 className="font-sans text-[#1E3A8A] font-black text-xl tracking-tight mb-1">
                          {top3[1].wardName}
                        </h3>
                      ...
                      <div className="w-20 h-20 shrink-0 relative rounded-full bg-slate-50 flex items-center justify-center border-4 border-slate-100/50 shadow-inner overflow-hidden">
                        <img
                          src={dinhImg}
                          alt="Icon"
                          className="w-16 h-16 object-contain opacity-90 drop-shadow-sm"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-5 divide-x divide-slate-100 mt-auto">
                      <StatMini
                        ...
                        value={`${top3[1].resolutionRate.toFixed(0)}%`}
                        highlight
                      />
                      <StatMini
                        label="Tốc độ xử lý (ngày)"
                        value={(top3[1].avgResolutionHours / 24).toFixed(1)}
                      />
                      <StatMini label="Tổng phản ánh" value={top3[1].totalFeedbacks.toString()} />
                      <StatMini
                        label="Đang xử lý"
                        value={(top3[1].totalFeedbacks - top3[1].resolvedCount).toString()}
                      />
                    </div>
                    ...
                    <button
                      onClick={() =>
                        navigate({
                          to: "/leaderboard/$wardId",
                          params: { wardId: top3[1].wardId.toString() },
                        })
                      }
                      className="flex items-center gap-1.5 text-sm font-bold text-[#2563EB] hover:text-[#1D4ED8] transition-colors"
                    >
                ...
                {top3[0] && (
                  <div className="bg-white rounded-[24px] border-2 border-[#FBBF24] shadow-lg shadow-amber-500/10 p-7 hover:shadow-xl hover:shadow-amber-500/20 transition-all relative overflow-hidden order-1 lg:order-2 h-[310px] flex flex-col lg:-mt-6 z-10">
                    ...
                    <div className="flex justify-between items-start mb-6 mt-2 ml-10">
                      <div>
                        <h3 className="font-sans text-[#1E3A8A] font-black text-[22px] tracking-tight mb-1">
                          {top3[0].wardName}
                        </h3>
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-6 divide-x divide-slate-100 mt-auto">
                      <StatMini
                        label="Tỷ lệ giải quyết"
                        value={`${top3[0].resolutionRate.toFixed(0)}%`}
                        highlight
                        highlightColor="text-[#D97706]"
                      />
                      <StatMini
                        label="Tốc độ xử lý (ngày)"
                        value={(top3[0].avgResolutionHours / 24).toFixed(1)}
                      />
                      <StatMini label="Tổng phản ánh" value={top3[0].totalFeedbacks.toString()} />
                      <StatMini
                        label="Đang xử lý"
                        value={(top3[0].totalFeedbacks - top3[0].resolvedCount).toString()}
                      />
                    </div>
                    ...
                    <button
                      onClick={() =>
                        navigate({
                          to: "/leaderboard/$wardId",
                          params: { wardId: top3[0].wardId.toString() },
                        })
                      }
                      className="flex items-center justify-center w-full gap-1.5 text-sm font-bold text-amber-600 bg-amber-50 hover:bg-amber-100 py-2.5 rounded-xl transition-colors"
                    >
                      Xem chi tiết <ArrowRight size={16} />
                    </button>
                  </div>
                ...
                {top3[2] && (
                  <div className="bg-white rounded-[18px] border border-slate-200 shadow-sm p-6 hover:shadow-md transition-shadow relative overflow-hidden order-3 h-[280px] flex flex-col">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-5 divide-x divide-slate-100 mt-auto">
                      <StatMini
                        label="Tỷ lệ giải quyết"
                        value={`${top3[2].resolutionRate.toFixed(0)}%`}
                        highlight
                        highlightColor="text-[#92400E]"
                      />
                      <StatMini
                        label="Tốc độ xử lý (ngày)"
                        value={(top3[2].avgResolutionHours / 24).toFixed(1)}
                      />
                      <StatMini label="Tổng phản ánh" value={top3[2].totalFeedbacks.toString()} />
                      <StatMini
                        label="Đang xử lý"
                        value={(top3[2].totalFeedbacks - top3[2].resolvedCount).toString()}
                      />
                    </div>
                    ...
                    <button
                      onClick={() =>
                        navigate({
                          to: "/leaderboard/$wardId",
                          params: { wardId: top3[2].wardId.toString() },
                        })
                      }
                      className="flex items-center gap-1.5 text-sm font-bold text-[#EA580C] hover:text-[#C2410C] transition-colors"
                    >
                      Xem chi tiết <ArrowRight size={16} />
                    </button>
                  </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
              ...
              <div className="w-full lg:w-[70%] bg-white rounded-[18px] border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                  <h2 className="font-sans text-lg font-black text-[#1E3A8A]">
                        ...
                        return (
                          <tr
                            ...
                            onClick={() =>
                              navigate({
                                to: "/leaderboard/$wardId",
                                params: { wardId: ward.wardId.toString() },
                              })
                            }
                          ...
                          >
                            <td className="px-6 py-4">
                              <div
                                className={`w-6 h-6 mx-auto rounded-full flex items-center justify-center text-xs font-black ${isRank1 || isRank2 || isRank3 ? `text-white ${badgeColor}` : ""}`}
                              >
                                {ward.rankPosition}
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4 text-center whitespace-nowrap">
                              <div className="flex items-center justify-center gap-1.5">
                                <span className="font-bold text-slate-700">
                                  {ward.resolutionRate.toFixed(0)}%
                                </span>
                                {ward.rankChange !== 0 && (
                                  <span
                                    className={`flex items-center text-[10px] font-bold ${ward.rankChange > 0 ? "text-emerald-500" : "text-red-500"}`}
                                  >
                                    {ward.rankChange > 0 ? (
                                      <TrendingUp size={10} />
                                    ) : (
              ...
              <div className="w-full lg:w-[30%] flex flex-col gap-6">
                ...
                <div className="bg-white rounded-[18px] border border-slate-200 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="font-sans font-black text-[#1E3A8A]">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex flex-col">
                      ...
                      <span className="text-[20px] font-black text-slate-800">
                        {totalReports.toLocaleString("vi-VN")}
                      </span>
                    ...
                    <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-3 flex flex-col">
                      ...
                      <span className="text-[20px] font-black text-slate-800">
                        {totalResolved.toLocaleString("vi-VN")}
                      </span>
                      <span className="text-[10px] font-bold uppercase text-slate-400">
                        Đã giải quyết
                      </span>
                ...
                <div className="bg-[#F8FAFC] rounded-[18px] border border-slate-200 p-5 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mb-3 font-medium leading-relaxed">
                      Dựa trên 3 tiêu chí chính với trọng số tương ứng: Tốc độ xử lý (40%), Tỷ lệ
                      giải quyết (40%) và Mức độ hài lòng (20%).
                    </p>
                    <button className="text-xs font-bold text-[#2563EB] hover:underline flex items-center gap-1">
                      Xem chi tiết <ArrowRight size={12} />
                    </button>
