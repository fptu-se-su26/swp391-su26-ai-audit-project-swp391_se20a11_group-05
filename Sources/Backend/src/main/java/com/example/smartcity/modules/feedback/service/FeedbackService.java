    ...
    public boolean canAccessFeedback(Feedback feedback, String username) {
        ...
        return switch (user.getRole()) {
            case SUPER_ADMIN ->
                true;
            case WARD_STAFF ->
                user.getWard() != null && user.getWard().getId().equals(feedback.getWard().getId());
            case POLICE ->
                feedback.getCategory() != null && "An ninh".equals(feedback.getCategory().getName());
            case CITIZEN ->
                feedback.getCitizen().getId().equals(user.getId());
        };
    }
