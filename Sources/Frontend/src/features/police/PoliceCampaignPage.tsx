...
export function PoliceCampaignPage() {
  const allCampaigns = useCampaignList();
  
  // Lọc chỉ lấy những chiến dịch mà Công an này đã tạo (có quyền quản lý)
  const campaigns = useMemo(() => {
    return allCampaigns.filter(c => c.canManage);
  }, [allCampaigns]);
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">
            Theo dõi và điều phối các chiến dịch cộng đồng của lực lượng Công an trên địa bàn.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          value={stats.total}
          note="Chiến dịch đã phát động"
          tone="blue"
        />
        <KpiCard
          icon={Users}
          label="Đang gọi đăng ký"
          value={stats.recruiting}
          note="Người dân đang tham gia"
          tone="amber"
        />
        <KpiCard
          icon={CheckCircle2}
          ...
          <table className="w-full min-w-[850px] text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70">
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Tên chiến dịch</th>
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Lĩnh vực</th>
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Thời gian</th>
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Trạng thái</th>
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Thành viên</th>
                <th className="px-5 py-3.5 text-xs font-extrabold uppercase tracking-wide text-slate-400">Thao tác</th>
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-4">
                      <p className="max-w-[280px] truncate text-sm font-extrabold text-[#0B2545]">{c.name}</p>
                      <p className="mt-1 max-w-[280px] truncate text-xs font-medium text-slate-400">{c.desc}</p>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-extrabold ${catInfo.bg}`}>{catInfo.label}</span>
                    </td>
                    <td className="px-5 py-4 text-xs font-medium text-slate-500">
                      {formatDate(c.startTime)} - {formatDate(c.endTime)}
                    </td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-extrabold ${statusInfo.bg}`}>{statusInfo.label}</span>
                    </td>
                    <td className="px-5 py-4">
                      {(() => {
                        ...
                        return (
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-extrabold ${isClosed ? "bg-slate-50 text-slate-600 border-slate-200" : isFull ? "bg-red-50 text-red-700 border-red-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}>
                    ...
                    <td className="px-5 py-4" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleViewDetail(c.id)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition" title="Xem chi tiết"><Eye size={15} /></button>
                        {c.canManage && <button onClick={() => handleEdit(c.id)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition" title="Chỉnh sửa"><Pencil size={14} /></button>}
                      </div>
                    </td>
                  </tr>
                );
              }) : (
                <tr><td colSpan={6} className="px-5 py-8 text-center text-slate-400 text-sm">Chưa có chiến dịch nào được phát động.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function KpiCard({ icon: Icon, label, value, note, tone = "blue" }: any) {
  const colors: any = {
  ...
  return (
    <section className="rounded-2xl border border-[#E4EAF2] bg-white p-5 shadow-sm flex items-start justify-between">
      ...
      <span className={`flex h-11 w-11 items-center justify-center rounded-xl border ${colors[tone]}`}><Icon size={20} /></span>
    </section>
  );
}

function QuickDetailsPanel({ campaign, onViewDetail }: any) {
    ...
    <div className="bg-white border border-[#E4EAF2] rounded-2xl p-5 shadow-sm flex flex-col h-[480px] justify-between">
      <div className="space-y-4 overflow-hidden">
        ...
        <div className="space-y-2 overflow-y-auto max-h-[190px] pr-1">
          ...
          <p className="text-slate-400 text-[11px] font-bold">Phường: {campaign.ward} | Tạo bởi: {campaign.createdBy}</p>
      ...
      <div className="space-y-3 pt-3 border-t border-[#E4EAF2] shrink-0">
        <div className="space-y-1.5 text-xs font-bold text-slate-700">
          <div className="flex items-center gap-2"><CalendarDays size={14} className="text-slate-400" /><span>Thời gian: {formatDate(campaign.startTime)} - {formatDate(campaign.endTime)}</span></div>
          <div className="flex items-center gap-2"><Users size={14} className="text-slate-400" /><span>Thành viên: {campaign.participants}/{campaign.target}</span></div>
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between gap-3"><span className="text-slate-400">Lĩnh vực:</span><span className={`inline-flex rounded-full border px-2.5 py-0.5 text-[11px] font-extrabold ${catInfo.bg}`}>{catInfo.label}</span></div>
            <div className="flex items-center justify-between gap-3"><span className="text-slate-400">Trạng thái:</span><span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-extrabold ${statusInfo.bg}`}>{statusInfo.label}</span></div>
          </div>
        </div>
        <button onClick={onViewDetail} className="w-full h-9 rounded-xl bg-[#0F5BD8] hover:bg-[#0B4FC0] text-white text-xs font-bold shadow-sm transition flex items-center justify-center gap-2 mt-2"><Eye size={14} />Xem chi tiết đầy đủ</button>
      </div>
    </div>
  );
}
