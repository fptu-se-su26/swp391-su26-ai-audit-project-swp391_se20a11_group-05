...
export function NewsManagement() {
  ...
  const { data: newsData, isLoading } = useNewsList(page, 10, selectedCategory || undefined, keyword || undefined);
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-[#E4EAF2]">
                    <th className="p-4 font-bold text-slate-600 text-[13px] uppercase tracking-wider">Tiêu đề & Tóm tắt</th>
                    <th className="p-4 font-bold text-slate-600 text-[13px] uppercase tracking-wider w-[150px]">Chuyên mục</th>
                    <th className="p-4 font-bold text-slate-600 text-[13px] uppercase tracking-wider w-[120px]">Lượt xem</th>
                    <th className="p-4 font-bold text-slate-600 text-[13px] uppercase tracking-wider w-[150px]">Ngày đăng</th>
                    <th className="p-4 font-bold text-slate-600 text-[13px] uppercase tracking-wider w-[100px] text-center">Thao tác</th>
                  </tr>
                </thead>
                    ...
                    <tr key={item.id} className="hover:bg-[#F8FAFD] transition-colors group">
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold border ${
                          item.category === "Thông báo" ? "bg-amber-50 text-amber-600 border-amber-200" :
                          item.category === "An ninh - Trật tự" ? "bg-red-50 text-red-600 border-red-200" :
                    ...
                    <tr>
                      <td colSpan={5} className="p-12 text-center">
                        <div className="flex flex-col items-center justify-center text-slate-400">
                          ...
                          <p className="text-sm mt-1">Hãy thử đổi từ khóa hoặc bấm "Thêm tin tức" để tạo bài viết mới.</p>
                        </div>
                      </td>
                    </tr>
                  )}
            ...
            {newsData && newsData.totalPages > 1 && (
              <div className="flex items-center justify-between mt-6 shrink-0">
                <span className="text-sm text-slate-500 font-medium">
                  Hiển thị trang <span className="font-bold text-slate-700">{page + 1}</span> / {newsData.totalPages}
                </span>
            ...
            <div className="px-6 pt-4 border-b border-[#E4EAF2] flex gap-4 shrink-0">
              <button
                ...
                className={`flex items-center gap-2 pb-3 font-bold text-sm border-b-2 transition-colors ${
                  activeTab === "edit" ? "border-[#0F5BD8] text-[#0F5BD8]" : "border-transparent text-slate-500 hover:text-slate-700"
                }`}
              >
                <Code size={16} />
                Soạn thảo
              ...
              <button
                ...
                className={`flex items-center gap-2 pb-3 font-bold text-sm border-b-2 transition-colors ${
                  activeTab === "preview" ? "border-[#0F5BD8] text-[#0F5BD8]" : "border-transparent text-slate-500 hover:text-slate-700"
                }`}
              >
                <Eye size={16} />
                Xem trước
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              {activeTab === "edit" ? (
                <form id="news-form" onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-[#0B2545] mb-1.5">Tiêu đề tin tức <span className="text-red-500">*</span></label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-slate-50 rounded-xl border border-slate-100">
                    <div>
                      <label className="block text-sm font-bold text-[#0B2545] mb-1.5">Chuyên mục <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-[#0B2545] mb-1.5">Tóm tắt ngắn <span className="text-red-500">*</span></label>
                    <textarea
                      required
                      rows={2}
                  ...
                  <div className="flex flex-col">
                    <label className="block text-sm font-bold text-[#0B2545] mb-1.5 flex justify-between items-end">
                      <span>Nội dung chi tiết <span className="text-red-500">*</span></span>
                      <span className="text-xs font-normal text-slate-400">Hỗ trợ định dạng Rich Text</span>
                    </label>
              ...
              {activeTab === "edit" && (
                ...
                >
                  {createMutation.isPending || updateMutation.isPending ? (
                    ...
                    editingId ? "Cập nhật" : "Đăng tin tức"
                  )}
                </button>
              )}
            </div>
