...
const mapCategoryName = (name: string | null | undefined): string => {
  ...
  if (n.includes("môi trường") || n.includes("environment") || n.includes("rác"))
    return "Môi trường";
  if (n.includes("an ninh") || n.includes("security") || n.includes("trật tự"))
...
export function OverviewPage() {
  ...
  const inProgressCount = feedbacks.filter((f) =>
    ["IN_PROGRESS", "ASSIGNED", "WAITING_INFO"].includes(f.status),
  ).length;
  ...
  const areaHotspots = useMemo(() => {
    const map: Record<
      string,
      { total: number; unresolved: number; overdue: number; cats: Record<string, number> }
    > = {};
    DEFAULT_WARDS.forEach((w) => {
      map[w] = { total: 0, unresolved: 0, overdue: 0, cats: {} };
    });
    feedbacks.forEach((fb) => {
      const w = DEFAULT_WARDS.find(
        (w) =>
    ...
    return DEFAULT_WARDS.map((name) => {
      ...
      return {
        ...
        overdue: d.overdue,
        unresolvedPct: d.total > 0 ? (d.unresolved / d.total) * 100 : 0,
        topCategory,
      };
    });
  }, [feedbacks]);
  ...
  const priorityReports = useMemo(() => {
    return feedbacks
      ...
      .map((fb) => {
        ...
        const ward =
          DEFAULT_WARDS.find((w) => fb.wardName?.toLowerCase().includes(w.toLowerCase())) ||
          "Đà Nẵng";
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                {kpi.trend && (
                  <span
                    className={`text-[10px] font-bold flex items-center gap-0.5 mt-1 ${kpi.trend.isUp ? "text-green-600" : "text-red-600"}`}
                  >
          ...
          <div className="space-y-3">
            {categoryCounts.map((cat) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-all ${isSelected ? "bg-[#0B4FC4] text-white" : "bg-blue-50 text-[#0B4FC4]"}`}
                  >
