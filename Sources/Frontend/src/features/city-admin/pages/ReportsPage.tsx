...
export function ReportsPage() {
  ...
  const resolutionRate =
    feedbacks.length > 0 ? Math.round((totalResolved / feedbacks.length) * 100) : 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Phân tích hiệu suất xử lý phản ánh toàn thành phố
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Phường hoạt động",
            value: wardPerf?.length ?? 0,
            icon: Award,
      ...
      <div className="bg-white rounded-2xl shadow-sm p-5">
        <h3 className="text-base font-bold text-[#0B4FC4] border-b border-slate-100 pb-3 mb-4">
          Hiệu suất xử lý theo phường
        </h3>
        {wardLoading ? (
                  ...
                  .map((ward, idx) => (
                    <tr key={ward.name} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <span className="px-2.5 py-0.5 bg-green-50 text-green-700 rounded-full text-xs font-bold">
                          {ward.resolved}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center text-sm font-bold text-slate-700">
                        {ward.satisfactionPct}%
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-[#0B4FC4] rounded-full transition-all"
                              style={{ width: `${ward.satisfactionPct}%` }}
                            />
                          </div>
                ...
                <tr key={cat.name} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${cat.rate >= 70 ? "bg-green-500" : cat.rate >= 40 ? "bg-amber-500" : "bg-red-400"}`}
