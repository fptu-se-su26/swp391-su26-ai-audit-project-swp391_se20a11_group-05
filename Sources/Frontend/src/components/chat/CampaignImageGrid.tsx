      ...
      <div className="mb-2 grid grid-cols-2 gap-1 max-w-[360px] rounded-lg overflow-hidden border border-slate-100/10 shadow-inner">
        {urls.map((url, index) => (
          <a
            ...
            className="relative aspect-square bg-slate-50/5"
          >
            <img
              src={url}
              alt={`Hình ảnh ${index + 1}`}
              className="h-full w-full object-cover hover:opacity-95 transition duration-200 cursor-zoom-in"
            />
          </a>
        ))}
      </div>
    ...
    return (
      <div className="mb-2 grid grid-cols-3 gap-1 max-w-[380px] rounded-lg overflow-hidden border border-slate-100/10 shadow-inner">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="col-span-2 aspect-[4/3] bg-slate-50/5"
        >
          <img
            src={urls[0]}
            alt="Hình ảnh 1"
        ...
        <div className="flex flex-col gap-1 col-span-1">
          {urls.slice(1).map((url, index) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="relative flex-1 aspect-square bg-slate-50/5"
            >
              <img
                src={url}
                alt={`Hình ảnh ${index + 2}`}
                className="absolute inset-0 h-full w-full object-cover hover:opacity-95 transition duration-200 cursor-zoom-in"
              />
            </a>
          ))}
        </div>
