...
import type { KpiData, WardPerformance, MonthlyTrend, DispatchAgency } from "@/types/api";
import {
  kpis as mockKpis,
...
export function CityAdminDashboard() {
  ...
  const currentKpis: {
    ...
    (hasApiData
      ...
      : {
          total: mockKpis.total,
          resolved: mockKpis.resolved,
          pending: mockKpis.pending,
          satisfactionRate: "94%",
        });
  ...
  return (
    ...
    >
        ...
        {[
          {
            ...
            val: currentKpis.total.toLocaleString(),
            border: "border-gov-blue",
            color: "text-gov-blue",
            spark: [42, 56, 48, 72, 65, 80, 78, 92, 88, 95, 102, 98],
          },
          {
            label: t("home.kpi.resolved"),
            val: currentKpis.resolved.toLocaleString(),
            border: "border-[var(--status-success)]",
            color: "text-[var(--status-success)]",
            spark: [30, 45, 38, 52, 48, 60, 58, 70, 68, 75, 82, 78],
          },
          {
            label: t("home.kpi.pending"),
            val: currentKpis.pending.toLocaleString(),
            border: "border-[var(--status-pending)]",
            color: "text-[var(--status-pending)]",
            spark: [18, 22, 15, 25, 20, 18, 28, 22, 16, 20, 18, 12],
          },
          {
            label: "Tỷ lệ hài lòng",
            val: analyticsKpi ? analyticsKpi.satisfactionRate : "Đang tính...",
            border: "border-gov-gold",
            color: "text-gov-blue",
            spark: [88, 90, 85, 92, 91, 94, 93, 95, 92, 94, 93, 94],
          },
        ].map((k, idx) => (
          <div
            key={k.label}
          ...
          >
            <div className="text-xs uppercase font-bold text-ink-soft tracking-wider mb-2">
              {k.label}
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        ...
        <HorizontalBarChart
          data={wardData.map((w: WardPerformance) => ({
            name: w.name || w.ward || "",
            value: w.resolved,
          }))}
              ...
              markers={
                apiFeedbacks.length > 0
                  ...
                  : [
                      ...
                      {
                        position: [16.07, 108.21],
                        title: "Phản ánh hạ tầng Hải Châu",
                        status: "inProgress",
                      },
                      {
                        position: [16.048, 108.215],
                        title: "Phản ánh môi trường Sơn Trà",
                        status: "pending",
                      },
                      {
                        position: [16.065, 108.24],
                        title: "Phản ánh an toàn Ngũ Hành Sơn",
                        status: "urgent",
                      },
                      {
                        position: [16.092, 108.175],
                        title: "Phản ánh giao thông Thanh Khê",
                        status: "resolved",
                      },
                      {
                        position: [16.04, 108.18],
                        title: "Phản ánh vệ sinh Cẩm Lệ",
                        status: "pending",
                      },
                    ]
              }
                ...
                >
                  <div className="flex justify-between mb-1.5">
                    ...
                    <span className="text-sm text-ink-soft">
                      {w.resolved} · {w.satisfactionPct ?? 0}%
                    </span>
                  </div>
                  <div className="h-3 rounded-full bg-slate-100 overflow-hidden">
                    <div
                      className="h-full bg-gov-blue"
                      style={{ width: `${(w.resolved / (maxResolved || 1)) * 100}%` }}
                    />
                  </div>
                </li>
              ))}
      ...
      <div className="mt-8 card-civic p-6 animate-slide-in-right">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {(
            ...
            <div
              key={p.name}
              className="border border-slate-200 rounded-xl p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
            >
