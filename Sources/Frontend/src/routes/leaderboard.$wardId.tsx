...
import {
  ArrowLeft, Share2, Download, Zap, ShieldCheck, Heart, Leaf, TrendingUp,
  MapPin, Clock, AlertCircle, FileText, CheckCircle2, Loader2, Award, ChevronLeft, ChevronRight,
  TrendingDown
} from "lucide-react";
import {
...
function getRankStyle(rank: number) {
  if (rank === 1) return { color: "#FBBF24", text: "HẠNG NHẤT", bg: "#FEF3C7", border: "#F59E0B", textFill: "#B45309" };
  if (rank === 2) return { color: "#94A3B8", text: "HẠNG NHÌ", bg: "#F1F5F9", border: "#64748B", textFill: "#334155" };
  if (rank === 3) return { color: "#FDE68A", text: "HẠNG BA", bg: "#FEF3C7", border: "#D97706", textFill: "#B45309" };
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "IN_PROGRESS" || status === "ASSIGNED") {
    return <span className="inline-flex px-2.5 py-1 rounded-md bg-amber-50 border border-amber-100 text-amber-700 text-xs font-semibold">Đang xử lý</span>;
  }
  if (status === "REJECTED") {
    return <span className="inline-flex px-2.5 py-1 rounded-md bg-slate-50 border border-slate-200 text-slate-700 text-xs font-semibold">Từ chối</span>;
  }
  return <span className="inline-flex px-2.5 py-1 rounded-md bg-blue-50 border border-blue-100 text-blue-700 text-xs font-semibold">Tiếp nhận</span>;
}
...
function CategoryBadge({ category }: { category: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 rounded-md border text-xs font-semibold ${bg}`}>{category}</span>;
}
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-20">
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-8 py-4 flex items-center justify-between">
        <Link to="/leaderboard" className="inline-flex items-center gap-2 text-[14px] font-semibold text-slate-500 hover:text-[#1E40AF] transition-colors">
          <ArrowLeft size={16} />
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-5 mb-6">
          ...
          <KPICard title="Tỷ lệ giải quyết" score={ward.resolutionRate} weight="30% trọng số" icon={ShieldCheck} color="text-[#16A34A]" barColor="bg-[#16A34A]" desc="Đã giải quyết đúng hạn" />
          <KPICard title="Hài lòng của người dân" score={ward.satisfactionScore} weight="20% trọng số" icon={Heart} color="text-[#EF4444]" barColor="bg-[#EF4444]" desc="Từ đánh giá người dân" />
          <KPICard title="Môi trường" score={ward.lowIncidenceScore} weight="10% trọng số" icon={Leaf} color="text-[#8B5CF6]" barColor="bg-[#8B5CF6]" desc="Phản ánh về môi trường" />
          <KPICard title="Cải thiện" score={ward.trendScore} weight="5% trọng số" icon={TrendingUp} color="text-[#F59E0B]" barColor="bg-[#F59E0B]" desc="Xu hướng cải thiện" />
        </div>
                ...
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                      ...
                      <Radar name={ward.wardName} dataKey="A" stroke="#2563EB" strokeWidth={2} fill="#3b82f6" fillOpacity={0.15} />
                      <Radar name="Trung bình thành phố" dataKey="B" stroke="#cbd5e1" strokeDasharray="4 4" strokeWidth={2} fill="transparent" />
                      <Legend wrapperStyle={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }} iconType="circle" />
                    </RadarChart>
                  </ResponsiveContainer>
                </div>
              </div>
