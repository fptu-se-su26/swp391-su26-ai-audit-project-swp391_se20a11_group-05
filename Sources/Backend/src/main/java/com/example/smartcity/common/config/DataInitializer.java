...
public class DataInitializer implements CommandLineRunner {
    ...
    private void ensureCampaignSchema() {
        ...
        executeSchemaSql("""
                ...
                """);
        executeSchemaSql("ALTER TABLE campaign_participants ADD COLUMN IF NOT EXISTS volunteer_experience VARCHAR(1000)");
        executeSchemaSql("ALTER TABLE campaign_participants ADD COLUMN IF NOT EXISTS availability_hours VARCHAR(200)");
        executeSchemaSql("ALTER TABLE campaign_participants ADD COLUMN IF NOT EXISTS cancellation_reason VARCHAR(500)");
        executeSchemaSql("ALTER TABLE campaign_participants ADD COLUMN IF NOT EXISTS cancel_count INTEGER DEFAULT 0");
        executeSchemaSql("UPDATE campaign_participants SET cancel_count = 0 WHERE cancel_count IS NULL");
        executeSchemaSql("ALTER TABLE campaign_participants ADD COLUMN IF NOT EXISTS confirmation_deadline TIMESTAMP");
        ...
        executeSchemaSql("""
                CREATE TABLE IF NOT EXISTS email_verifications (
                    ...
                    purpose VARCHAR(30) DEFAULT 'CAMPAIGN_JOIN',
                ...
                """);
        executeSchemaSql("CREATE INDEX IF NOT EXISTS idx_email_is_used ON email_verifications(email, is_used)");
